...
export default function Contact({ subject = undefined, rows = 10 }) {
    const [confirmation, setConfirmation] = useState<string | undefined>(undefined);
    let from = useSearchParams()?.get('from')
    let formSubject = subject ? `subject: ${subject}` : `enquiry topic: ${from ? from?.slice(1) : 'general enquiry'}`;
    ...
    return <>
        ...
        <div className="content" id="contact">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div>
                    {errors.email?.type === 'required' && errors.email.message && <p className="error">{errors.email.message}</p>}
                    {errors.email?.type === 'pattern' && errors.email.message && <p className="error">{errors.email.message}</p>}
                    <input
                        id="email"
                        type="text"
                        placeholder="email"
            ...
            <p><i>Developer note: This form uses react-hook-form for clientside validaton and server actions to
                ...
                actions act as a secure proxy between the client and the CMS</i></p>
